...
export const siteConfig: SiteConfig = {
  // Site Information
  // [CONFIG:SITE_URL]
  site: "https://arbeo.dev",
  // [CONFIG:SITE_TITLE]
  title: "Connecto ergo sum",
  // [CONFIG:SITE_DESCRIPTION]
  description: "Non connecto ergo urgueo.",
  // [CONFIG:SITE_AUTHOR]
  author: "Sergio Arbeo",
  ...
  seo: {
    ...
    defaultOgImageAlt: "", // Alt text for the default Open Graph image, public/open-graph.png
  },
  deployment: {
  ...
  homeOptions: {
    featuredPost: {
      ...
      slug: "better-tests-zero-drama-smarter-liveisolatedcomponent-patterns", // Slug of post after '/posts/' to be featured (e.g. "post-title"). Only used when type is "featured"
    },
    recentPosts: {
...
function validateSiteConfig(config: SiteConfig): {
  ...
  if (!config.site || !config.site.startsWith("http")) {
    errors.push(
      'Site URL is missing or invalid. Please set a complete URL like "https://yourdomain.com" in the site field.'
    );
  }
  if (!config.title || config.title.trim() === "") {
    errors.push(
      "Site title is required and cannot be empty. Set a descriptive title for your blog."
    );
  }
  if (!config.description || config.description.trim() === "") {
    errors.push(
      "Site description is required and cannot be empty. Set a brief description of your blog."
    );
  }
  ...
  if (!validThemes.includes(config.theme)) {
    errors.push(
      `Invalid theme selected: "${config.theme}". Choose from: Minimal, Oxygen, Atom, Ayu, Catppuccin, Charcoal, Dracula, Everforest, Flexoki, Gruvbox, macOS, Nord, Obsidian, Rose Pine, Sky, Solarized, or Things. For custom themes, use "custom" and set customThemeFile.`
    );
  }
  if (config.theme === "custom") {
    if (!config.customThemeFile || config.customThemeFile.trim() === "") {
      errors.push(
        'Custom theme file is required when theme is set to "custom". Set customThemeFile to the filename (without .ts extension) in src/themes/custom/'
      );
    }
  }
  ...
  if (Array.isArray(config.availableThemes)) {
    if (config.availableThemes.length === 0) {
      errors.push(
        'availableThemes array cannot be empty. Use "default" to show all built-in themes or specify theme names in the array.'
      );
    }
  ...
  if (!["local", "cdn"].includes(config.fonts.source)) {
    errors.push(
      `Font source must be either "local" (self-hosted fonts) or "cdn" (Google Fonts). Current value "${config.fonts.source}" is invalid.`
    );
  }
  ...
  if (!config.fonts.families.mono || config.fonts.families.mono.trim() === "") {
    errors.push(
      'Monospace font family is required. Set fonts.families.mono to a valid font name like "JetBrains Mono", "Fira Code", or "Source Code Pro".'
    );
  }
  if (!["swap", "fallback", "optional"].includes(config.fonts.display)) {
    errors.push(
      'Font display strategy must be one of: "swap", "fallback", or "optional". Current value is invalid.'
    );
  }
  ...
  if (config.tableOfContents.depth < 2 || config.tableOfContents.depth > 6) {
    errors.push(
      `Table of contents depth must be between 2 and 6 (where 2=H2, 3=H3, etc.). Current value is ${config.tableOfContents.depth}. Adjust tableOfContents.depth.`
    );
  }
  ...
  if (!["minimal", "traditional"].includes(config.navigation.style)) {
    errors.push(
      `Navigation style must be either "minimal" or "traditional". Current value "${config.navigation.style}" is invalid.`
    );
  }
  ...
  if (!["latest", "featured"].includes(config.homeOptions.featuredPost.type)) {
    errors.push(
      `Featured post type must be either "latest" or "featured". Current value "${config.homeOptions.featuredPost.type}" is invalid.`
    );
  }
  ...
  if (!config.language || !config.language.match(/^[a-z]{2}(-[A-Z]{2})?$/)) {
    errors.push(
      'Language must be a valid language code like "en" or "en-US". Current value is invalid.'
    );
  }
  ...
  if (config.profilePicture.enabled) {
    ...
    if (!config.profilePicture.alt || config.profilePicture.alt.trim() === "") {
      errors.push(
        "Profile picture alt text is required when profilePicture.enabled is true. Set profilePicture.alt to describe your profile picture for accessibility."
      );
    }
    if (!["sm", "md", "lg"].includes(config.profilePicture.size)) {
      errors.push(
        `Profile picture size must be "sm" (32px), "md" (48px), or "lg" (64px). Current value "${config.profilePicture.size}" is invalid.`
      );
    }
    if (!["footer", "header"].includes(config.profilePicture.placement)) {
      errors.push(
        `Profile picture placement must be "footer" or "header". Current value "${config.profilePicture.placement}" is invalid.`
      );
    }
    if (!["circle", "square", "none"].includes(config.profilePicture.style)) {
      errors.push(
        `Profile picture style must be "circle", "square", or "none". Current value "${config.profilePicture.style}" is invalid.`
      );
    }
    if (
      config.profilePicture.url &&
      !config.profilePicture.url.startsWith("/") &&
      !config.profilePicture.url.startsWith("http")
    ) {
      errors.push(
        `Profile picture URL must be a valid path starting with "/" or "http". Current value "${config.profilePicture.url}" is invalid.`
      );
    }
...
if (!validation.isValid) {
  throw new Error(
    `Site configuration is invalid. Please fix the following issues:\n${validation.errors.join(
      "\n"
    )}`
  );
}
